...
let buttonName = null;   // new
let eraserValue = null;    // new
...
function changeProperty(propName, propValue) {
    ...
    }else if(propName == 'eraser') {  //new change
        eraserValue = propValue; //new
    }else
...
function clear(e) {   // new
        ctx.clearRect(e.clientX - position.left - (eraserValue / 2), e.clientY - position.top - (eraserValue / 2), eraserValue, eraserValue);
};
